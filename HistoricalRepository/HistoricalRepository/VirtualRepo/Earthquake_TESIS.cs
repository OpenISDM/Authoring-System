...
{
    static class Earthquake_TESIS
    {
        public static List<EarthquakeObj> GetData(string startdate, string enddate, string min_magnitude, string max_magnitude)
        {
            string data = null;
            string requestString = RequestProducer.GetEarhquakeRequestString(EarthquakeDataSource.TESIS, startdate, enddate, min_magnitude, max_magnitude);
            ...
            using (var response = (HttpWebResponse)request.GetResponse())
            {
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    using (var reader = new StreamReader(stream))
                    {
                        ...
                        if (data != "false")
                        {
                            EqList = EarthquakeObjConverter.EarthquakeTrans(data, EarthquakeDataSource.TESIS, startdate, enddate);
                        }
                        reader.Close();
                        reader.Dispose();
                    }
